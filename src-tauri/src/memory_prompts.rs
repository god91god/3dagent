...
/// 近期对话压缩 prompt（借鉴 N.E.K.O. memory/recent.py 的 summary prompt 思路）
/// 用途: 旧对话轮数超阈值时，LLM 把 N 条历史消息压成一段"过去对话备忘录"，
///       保留重要信息（主人说过的事/承诺/计划/偏好），丢弃寒暄与流水账
/// 占位符: {CONVERSATION}
pub const RECENT_COMPRESS_PROMPT: &str = r#"你是对话归档助手。把下面这段过去的对话压缩成一段简洁的"对话备忘录"，保留之后可能用到的信息：

保留：
- 主人说过的重要个人信息、计划、承诺、偏好、正在做的事
- 优香答应过要做但还没做的事
- 值得记住的关系动态（约好一起做某事、重要约定）
- 事件要点（谁、什么时候、发生了什么）
...
要求：
- 输出一段流畅的中文短文（200字以内），不要列表格式，不要复述原文
- 按时间顺序组织：先发生的在前
- 不确定是否重要的宁可保留一句简短的
- 直接输出备忘录文本，不要任何格式标记
